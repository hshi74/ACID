...
def export_train_data(data_id):
    ...
    grasp_loc, target_loc, f1, _, f2 = plushsim_util.get_action_info(
    ...
    obj_pts1 = common_util.subsample_points(
        common_util.transform_points(obj_pts1, scene_range, to_range),
        resolution=0.005,
        return_index=False,
    )
    env_pts1 = common_util.subsample_points(
        common_util.transform_points(env_pts1, scene_range, to_range),
        resolution=0.020,
        return_index=False,
    )
    # calculate flow
    ...
    pts, occ, pt_class = plushsim_util.sample_occupancies(
        sim_pts1, center, std=class_to_std[model_category], sample_scheme="object"
    )
    # get implicit flows
    flow = sim_pts_flow[pt_class]
    # save
    kwargs = {
        "sim_pts": sim_pts1.astype(np.float16),
...
def get_all_data_points_flow(data_root):
    ...
    for g in tqdm.tqdm(good_interactions):
        ...
        model_dir = plushsim_util.get_model_dir(
        ...
        for k, v in good_ones.items():
            ...
            for int_id in v:
                good_ints.append(
                    (split_id, model_category, model_name, reset_id, int_id)
                )
    return good_ints
...
with Pool(40) as p:
    for _ in tqdm.tqdm(
        p.imap_unordered(export_train_data, good_ints), total=len(good_ints)
    ):
        pass
