import os
from ACID.src.encoder import encoder_dict
from ACID.src.conv_onet import models, training
from ACID.src.conv_onet import generation
from ACID.src import data
...
def get_combined_model(cfg, device=None, dataset=None, **kwargs):
    ...
    if "env_c_dim" in cfg["model"] and "env_c_dim" != 0:
        ...
        env_encoder = encoder_dict[encoder](
            dim=dim, c_dim=env_c_dim, padding=padding, **env_encoder_kwargs
        )
    else:
        env_c_dim = 0
        env_encoder = None
    ...
    obj_per_encoder = encoder_dict[encoder](
        dim=dim, c_dim=obj_c_dim, padding=padding, **obj_encoder_kwargs
    )
    obj_act_encoder = encoder_dict[encoder](
        dim=act_dim, c_dim=obj_c_dim, padding=padding, **obj_encoder_kwargs
    )
...
def get_geom_model(cfg, device=None, dataset=None, **kwargs):
    ...
    decoder = models.decoder_dict[decoder](
        dim=dim, c_dim=obj_c_dim + env_c_dim, padding=padding, **decoder_kwargs
    )
    ...
    model = models.ConvOccGeom(obj_encoder, env_encoder, decoder, device=device)
...
def get_trainer(model, optimizer, cfg, device, **kwargs):
    ...
    trainer = training.PlushTrainer(
        model, optimizer, cfg, device=device, vis_dir=vis_dir
    )
