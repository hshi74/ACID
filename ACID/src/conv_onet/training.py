...
from ACID.src.common import compute_iou
from ACID.src.utils import common_util, plushsim_util
from ACID.src.training import BaseTrainer
...
from ACID.src.utils.plushsim_util import find_nn_cpu, find_emd_cpu
...
class PlushTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if (
            "corr_dim" in cfg["model"]["decoder_kwargs"]
            and cfg["model"]["decoder_kwargs"]["corr_dim"] > 0
        ):
            ...
            self.contrastive_coeff_neg = cfg["loss"].get("contrastive_coeff_neg", 1.0)
            self.contrastive_neg_thres = cfg["loss"].get("contrastive_neg_thres", 1.0)
            self.contrastive_coeff_pos = cfg["loss"].get("contrastive_coeff_pos", 1.0)
    ...
    def evaluate(self, val_loader):
        ...
        if "fmr_hits" in agg_list:
            ...
            for tau2 in np.linspace(0.01, 0.2, 5):
                figs[f"FMR_tau1_wrt_tau2={tau2:.3f}"] = self._get_FMR_curve_tau1(
                    fmr, tau2=tau2
                )
            figs["FMR_tau1"] = self._get_FMR_curve_tau1(fmr)
            for tau1 in np.linspace(0.01, 0.1, 5):
                figs[f"FMR_tau2_wrt_tau1={tau1:.3f}"] = self._get_FMR_curve_tau2(
                    fmr, tau1=tau1
                )
    ...
    def _get_pair_distance_histogram(self, all_dists):
        ...
        counts, bins, patches = ax.hist(
            all_dists, density=True, bins=40
    ...
    def eval_step(self, data):
        ...
        with torch.no_grad():
            ...
            if "flow" in outputs:
                ...
                constant = (
                    torch.from_numpy(
                        np.array((12.0, 12.0, 4.0)) / 10.0 / (1.1, 1.1, 1.1)
                    )
                    .float()
                    .cuda()
                )
                loss_flow = F.mse_loss(
                    outputs["flow"] * constant, gt_flow * constant, reduction="none"
                )
            ...
            if "flow" in outputs:
                flow_vis_mean = []
                for i in range(B * 2):
                    gt_occ_pts = gt_pts[i][occ_iou_np[i]] * (1200, 1200, 400) / (
                        1.1,
                        1.1,
                        1.1,
                    ) + (0, 0, 180)
                    vis_idx = plushsim_util.render_points(
                        gt_occ_pts,
            ...
            if idx % 10000 == 9999:
                ...
                # occupancy ROC curve
                fpr, tpr, _ = roc_curve(
                    occ_iou_np.flatten(), outputs["occ"].probs.cpu().numpy().flatten()
                )
                ...
                f1 = []
                for i in range(B * 2):
                    gt_occ_pts = common_util.subsample_points(
                        gt_pts[i][occ_iou_np[i]], return_index=False
                    )
                    pred_pts = common_util.subsample_points(
                        gt_pts[i][occ_iou_hat_np[i]], return_index=False
                    )
    ...
    def compute_loss(self, data, it):
        ...
        if "flow" in outputs:
            ...
            loss["flow"] = F.mse_loss(flow_pred_1, flow_gt_1) + 0.01 * F.mse_loss(
                flow_pred_0, flow_gt_0
            )
        ...
        if "corr" in outputs:
            ...
            if self.loss_type == "contrastive":
                if num_positive > 0:
                    ...
                    # Positive loss
                    pos_loss = F.relu(
                        ((src_pos - tgt_pos).pow(2).sum(1) + 1e-4).sqrt()
                        - self.contrastive_pos_thres
                    ).pow(2)
                ...
                # Negative loss
                neg_dist = (
                    dist_vec[dist_vec > self.contrastive_threshold]
                    / self.contrastive_threshold
                ).log() + 1.0
                neg_dist = torch.clamp(neg_dist, max=2)
                neg_loss = F.relu(
                    neg_dist - ((src_neg - tgt_neg).pow(2).sum(1) + 1e-4).sqrt()
                ).pow(2)
