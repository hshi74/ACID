...
from ACID.src.layers import ResnetBlockFC
from ACID.src.common import normalize_coordinate, normalize_3d_coordinate, map2local
        ...
        for i in range(self.n_blocks):
            ...
            if self.fuse:
                net_act = self.blocks_act[i](
                    self.fc_c_merge[i](torch.cat((net_act, per_features[i]), dim=-1))
                )
