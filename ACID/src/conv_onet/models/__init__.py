...
from ACID.src.conv_onet.models import decoder
from ACID.src.utils import plushsim_util
...
class ConvImpDyn(nn.Module):
    def __init__(
        ...
        if env_encoder is None:
            ...
            self.env_upsample = torch.nn.UpsamplingBilinear2d(
                scale_factor=env_scale_factor
            )
        self._device = device
    ...
    def rollout(self, pts, per_features, c_env, actions):
        ...
        for j in range(num_actions):
            act_traj = []
            points_world = [
                p.cpu().numpy().squeeze() * (1200, 1200, 400) / (1.1, 1.1, 1.1)
                + (0, 0, 180)
                for p in curr_pts
            ]
            for i in range(num_sequence):
                ...
                start_time = time.time()
                flow = self.decoder.decode_action(curr_pts[i], c_act, per_features)[
                    "flow"
                ]
                curr_pts[i] = curr_pts[i] + flow / 10.0
    ...
    def rollout_async(self, pts, per_features, c_env, actions):
        ...
        for j in range(num_actions):
            ...
            for xyz, act in zip(xyz_chunks, act_chunks):
                obj_xyz = torch.as_tensor(np.stack(xyz).astype(np.float32)).to(
                    self._device
                )
                obj_act = torch.as_tensor(np.stack(act).astype(np.float32)).to(
                    self._device
                )
                ...
                for chunk_i in range(len(xyz)):
                    ...
                    for k in c_act_new.keys():
                        c_act[k] = torch.cat(
                            [c_act_new[k][chunk_i].unsqueeze(0), c_env[k]], dim=1
                        )
                    c_acts.append(c_act)
            total_time_act_decode += time.time() - start_time
            from ACID.src.utils import common_util
            ...
            pdb.set_trace()
            for i in range(num_sequence):
                flow = self.decoder.decode_action(curr_pts[i], c_acts[i], per_features)[
                    "flow"
                ]
                curr_pts[i] = curr_pts[i] + flow / 10.0
    ...
    def get_action_encoding_new(self, pts, grasp_loc, target_loc, c_env, vis_idx=None):
        ...
        # make action feature
        tiled_grasp_loc = np.tile(grasp_loc.cpu().numpy(), (len(obj_xyz), 1)).astype(
            np.float32
        )
        tiled_target_loc = np.tile(target_loc.cpu().numpy(), (len(obj_xyz), 1)).astype(
            np.float32
        )
...
class ConvOccGeom(nn.Module):
    ...
    def __init__(
        self, obj_encoder, env_encoder, decoder, device=None, env_scale_factor=2.0
    ):
        super().__init__()
