...
import trimesh
from ACID.src.utils import libmcubes, common_util
from ACID.src.common import make_3d_grid, normalize_coord, add_key, coord2index
from ACID.src.utils.libmise import MISE
import time
import math
...
class Generator3D(object):
    ...
    def extract_mesh(self, occ_hat, c=None, stats_dict=dict()):
        ...
        mesh = trimesh.Trimesh(
            vertices / (1.0, 1.0, 3), triangles, vertex_normals=None, process=False
        )
        ...
        if self.simplify_nfaces is not None:
            t0 = time.time()
            from ACID.src.utils.libsimplify import simplify_mesh
    ...
    def generate_pointcloud(self, data, threshold=0.75, use_gt_occ=False):
        ...
        if len(flows) != 0:
            flow = torch.cat(flows, dim=1).numpy() / 10.0
            pos_f0 = flow[0][occ_hat[0] == 1.0].reshape((-1, 3))
            pos_f1 = flow[1][occ_hat[1] == 1.0].reshape((-1, 3))
            pos_f0 = pos_f0[pos_idx0]
            pos_f1 = pos_f1[pos_idx1]
            pcloud_unroll_0 = pcloud0.copy()
            pcloud_unroll_0[:, :3] += pos_f0 / (1.0, 1.0, 3.0)
            pcloud_unroll_1 = pcloud1.copy()
            pcloud_unroll_1[:, :3] += pos_f1 / (1.0, 1.0, 3.0)
