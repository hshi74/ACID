...
from ACID.src.utils import plushsim_util, common_util
...
class PlushEnvBoth(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            (
                ...
                sampled_flow,
                sampled_inds,
            ) = self._prepare_points(points_dict)
            ...
            sampled_occs_ids = total_to_occs[sampled_inds[sampled_occ == 1.0]]
            # basically sampled_positive ids is used to index the pairs in pair info npz
            # reorganize sampled_pts
            sampled_pts = np.concatenate(
                [sampled_pts[sampled_occ == 1.0], sampled_pts[sampled_occ == 0.0]]
            )
            sampled_occ = np.concatenate(
                [sampled_occ[sampled_occ == 1.0], sampled_occ[sampled_occ == 0.0]]
            )
            sampled_flow = np.concatenate(
                [sampled_flow[sampled_occ == 1.0], sampled_flow[sampled_occ == 0.0]]
            )
            geo_dists, tgtids = self._prepare_pair_data(pair_info, sampled_occs_ids)
            _, _, sampled_pts2, sampled_occ2, sampled_flow2, _ = self._prepare_points(
                points_dict2, chosen=tgtids
            )
        else:
            ...
            (
                ...
                sampled_flow2,
                geo_dists,
            ) = self._prepare_matched_unique(points_dict, points_dict2)
    ...
    def _get_pts_related_info(self, points_dict):
        ...
        flow[occs != 0] = points_dict["flow"].astype(np.float32) * 10.0
    ...
    def _prepare_matched_unique(self, points_dict, points_dict2):
        ...
        int_cls, int_id1, int_id2 = np.intersect1d(
            cls1, cls2, assume_unique=True, return_indices=True
        )
        ...
        sub_inds = common_util.subsample_points(
            unique_pts_1, resolution=0.03, return_index=True
        )
        unique_pts_1 = unique_pts_1[sub_inds]
        unique_flow_1 = unique_flow_1[sub_inds]
        unique_occ_1 = unique_occ_1[sub_inds]

        sample_others1 = np.random.randint(
            pts1.shape[0], size=pts1.shape[0] - unique_pts_1.shape[0]
        )
        ...
        sample_others2 = np.random.randint(
            pts2.shape[0], size=pts2.shape[0] - unique_pts_2.shape[0]
        )
    ...
    def _prepare_partial_obs(self, info_dict):
        ...
        grasp_loc = common_util.transform_points(
            info_dict["grasp_loc"], scene_range, to_range
        )
        target_loc = common_util.transform_points(
            info_dict["target_loc"], scene_range, to_range
        )
        ...
        obj_pcloud = np.concatenate(
            [obj_pcloud, tiled_target_loc, obj_pcloud[:, :3] - tiled_grasp_loc], axis=-1
        )
    ...
    def _prepare_points(self, points_dict, chosen=None):
        ...
        if chosen is None:
            ...
            if self.num_points == -1:
                sample_others = np.random.randint(
                    pts.shape[0], size=pts.shape[0] - chosen.shape[0]
                )
            else:
                sample_others = np.random.randint(
                    pts.shape[0], size=self.num_points - chosen.shape[0]
                )
    ...
    def _prepare_pair_data(self, pair_info, sampled_occs_ids):
        ...
        draw_pair_ids[H // self.pos_ratio :] = np.random.randint(
            3, high=W, size=H - H // self.pos_ratio
        )
...
class PlushEnvGeom(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            # if training, load random points
            # implicit network sampled points
            pts, occs, sampled_pts, sampled_occ, sampled_inds = self._prepare_points(
                points_dict
            )
            # get which occupied points are sampled (index is in the occupied subset)
            occed = occs != 0
            ...
            geo_dists, tgtids = self._prepare_pair_data(pair_info, sampled_occs_ids)
            _, _, sampled_pts2, sampled_occ2, _ = self._prepare_points(
                points_dict2, chosen=tgtids
            )
        else:
            # if not training, load matched points
...
def get_plush_loader(
    cfg, mode, split="train", transform="build", test_shuffle=False, num_workers=None
):
    if mode == "geom":
...
def get_plan_loader(cfg, transform="build", category="teddy", num_workers=None):
    transform = build_transform_geom(cfg)
    dataset = PlushEnvPlan(
        cfg["data"]["plan_path"], category=category, transform=transform
    )
    loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=1,
