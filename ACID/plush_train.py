...
from ACID.src import config, data
from ACID.src.checkpoints import CheckpointIO
...
# Arguments
parser = argparse.ArgumentParser(description="Train a Plush Env dynamics model.")
...
if args.eval_only:
    ...
    for data_vis in data_vis_list:
        ...
        pcloud_dict = [
            {
                "title": "source" if i == 0 else "target",
                "pts": p[:, :3],
...
while True:
    ...
    for batch in train_loader:
        ...
        if (it % print_every) == 0:
            ...
            print_str = (
                f"[Epoch {epoch_it:04d}] it={it:04d}, time: {time.time()-t0:.3f}, "
            )
        ...
        if visualize_every > 0 and (it % visualize_every) == 0:
            ...
            for data_vis in data_vis_list:
                ...
                try:
                    ...
                    mesh, stats_dict = out, {}
                mesh.export(
                    os.path.join(out_dir, "vis", "{}_{}.off".format(it, data_vis["it"]))
                )
                out2 = generator.generate_pointcloud(data_vis["data"])
                for i, pcloud in enumerate(out2):
                    ipath = os.path.join(
                        out_dir, "vis", f"{it}_{data_vis['it']}_{i}.obj"
                    )
                    common_util.write_pointcoud_as_obj(ipath, pcloud)
